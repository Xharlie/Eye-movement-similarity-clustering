...
int main(int argc, char *argv[]) {
	...
	sprintf(fname_vx, "vx_s_%d.txt", video_num);
	sprintf(fname_vy, "vy_s_%d.txt", video_num);
	...
	for(tau = 1; tau <= tau_max ; ++tau) {
		...
		sprintf(fname_corr, "corr_corr_split_video%d.txt", video_num);
		sprintf(fname_mag, "mag_mag_split_video%d.txt", video_num);
		sprintf(fname_J, "Jinf_split_video%d.txt", video_num);
		sprintf(fname_H, "Hinf_split_video%d.txt", video_num);
		...
		if( (maxDiff_C < eps) || (tau == tau_max)) {
			...
			sprintf(fname_Cv, "Cv_split_video%d.txt", video_num);
			...
			for(beta = 0.7; beta <= 6.0; beta += 0.05) {
                ...
                chi = (M_square - Mag*Mag)*beta; //pow(beta,2.0);
